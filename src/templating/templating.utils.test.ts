...
import { TokenType } from "./templating.utils";
describe("templating.utils", () => {
  ...
  describe("constructTree", () => {
    ...
    it("should process plain text tokens", () => {
      const tokens: TokenType[] = [{ type: "text", value: "Hello, world!" }];
    ...
    it("should handle nested tags", () => {
      ...
      const result = [
        ...
        {
          type: "b",
          content: ["two ", { type: "r", content: ["three"] }, " four"],
        },
        " five",
      ];
      expect(SUT.constructTree(tokens)).toEqual([result, 9]);
    });
  });
  describe("parse", () => {
    ...
    it("should return an empty array if only orphan tokens are provided", () => {
      const input = "{b}hello{/r} world {r}test";
      const result = SUT.parse(input);
      expect(result).toEqual([]);
    });
